...
#' @param add logical; indicates whether to start a new plot, \code{TRUE} by default.
...
#' @export
plot.citfun <- function(x, ..., xmarg=10, add=F, xlab="", ylab="", main="", ylim=c(0, max(x)), xlim=c(0,length(x)+xmarg))
{
	n <- length(x);
